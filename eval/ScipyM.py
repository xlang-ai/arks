...
def Scipy_modified_easy_worker(arguments):
...
def Scipy_modified_easy_eval(output_dir,turn=-1,num_workers=-1,show_error_source=False):
    judge_arguments = [(os.path.join(output_dir, d),turn,show_error_source) for d in os.listdir(output_dir) if os.path.isdir(os.path.join(output_dir,d)) and d.startswith('q')]
    correct = 0
    total = len(judge_arguments)
    with_log = 0
    with_result = 0
    syntax_incorrect = []
    syntax_correct = []
    ...
    with mp.Pool(num_workers) as pool, tqdm(total=len(judge_arguments), desc='Evaluate') as pbar:
        for result in pool.imap_unordered(Scipy_modified_easy_worker, judge_arguments):
            pbar.update()
            if result[0]:
                correct += 1
...
if __name__=='__main__':
    ...
    Scipy_modified_easy_eval(output_dir=args.output_dir,num_workers=args.num_workers,turn=args.turn,
